...
TEST_F(NodeMetaTest, NodeMeta)
{
    ...
    // group id
    uint32_t group_id = cfg::GetSentenceIndex(config::ParagraphType::GROUP, "POS_IO");
    EXPECT_EQ(2, group_id);
    ...
    EXPECT_EQ(node[0].group_id, cfg::GetSentenceIndex(config::ParagraphType::GROUP, "POS_IO"));
}
