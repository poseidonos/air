...
ex1_function(const air::JSONdoc&& data)
{
    ...
    if (data.HasKey("PERF_BENCHMARK"))
    {
        ...
        for (auto& obj_it : objs)
        {
            ...
            for (auto& item : obj)
            {
                std::cout << "  " << item.first << ":" << obj[item.first]
                          << std::endl;
            }
            std::cout << "}\n";
        }
...
Delegator::Test(void)
{
    ...
    air_request_data({"PERF_BENCHMARK"}, [this](const air::JSONdoc&& data) -> int {
        usleep(200000); // 200ms
    ...
    air_request_data(
        {"LAT_IO_PATH"}, [copy = *this](const air::JSONdoc&& data) mutable -> int {
            usleep(300000); // 300ms
